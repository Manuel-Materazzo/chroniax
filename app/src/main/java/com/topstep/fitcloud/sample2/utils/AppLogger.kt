...
object AppLogger {
    ...
    private fun initXlog(context: Context, logDir: File): Xlog? {
        return try {
            ...
            xlog.setMaxFileSize(0, 10 * 1024 * 1024) // 10MB max file size
            xlog
        } catch (e: UnsatisfiedLinkError) {
            Timber.tag("AppLogger").e("Failed to initialize XLog: %s", e.message)
            Timber.tag("AppLogger").w("Falling back to Android logging for T10 smartwatch")
            null
        }
    }
